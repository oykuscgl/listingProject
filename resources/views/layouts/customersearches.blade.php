    ...
    <main class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
        @foreach($researches as $research)
            <div class="relative bg-black shadow-md rounded-lg overflow-hidden full-height">
                <img class="w-full filter brightness-75"  alt="{{ $research->title }}">
                <div class="gradient-overlay">
                    <h2 class="lg-title text-white text-4xl font-bold mb-4">{{ $research->title }}</h2>
                    <p class="text-gray-700 mb-4 text-2xl lg-text">{{ $research->description }}</p>
                    <a href="{{ route('researches.show', $research->id) }}" class="btn mt-4">Keşfet <i class="fas fa-angle-right"></i></a>
                </div>
            </div>
        @endforeach
    </main>
