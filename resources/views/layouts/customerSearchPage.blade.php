...
@section('content')
    ...
    <div class="relative mr-2">
        <img class="w-full object-cover banner"  src="{{ asset('storage/' . $research->image) }}" alt="Banner Image"
            style="height:30vh">
        ...
        <div class="absolute top-1/3 left-12 transform -translate-y-1/2">
            <div class="flex items-center space-x-2 text-sm text-black">
                ...
                <p class="text-red-500">{{ $research->title }}</p>
            </div>
            <div>
                <h3 class="font-bold text-red-700 text-4xl mt-5">{{ $research->title }}</h3>
            </div>
        </div>
    </div>
